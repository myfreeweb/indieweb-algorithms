...
parseTwitter s r = if isJust tweet && isJust username && isJust tweetText
  ...
  where wrapper = object [ "type" .= [ String "h-entry" ]
        ...
        profileUrl _ = Null
        tweetText = tweet >>= (^? hasClass "tweet-text") >>= return . extractProperty def "e"
        ...
        permalink = tweet >>= (^? hasClass "tweet-timestamp") >>= return . extractProperty def "u"
